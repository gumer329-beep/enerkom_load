...
def main():
    ...
    if cleaner is not None:
        df= cleaner
        print(f"🧹 Ejecutando limpieza de datos para '{table_name}' usando '{cleaner}'...")
        # Ejecutar pipeline ETL
        run_pipeline(df, config, table_name, verbose=True)
    else:
        raise NotImplementedError(f"No hay función de limpieza definida para '{table_name}' no se cargará en la base de datos.")
